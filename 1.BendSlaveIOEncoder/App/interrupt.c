...
//extern unsigned char enc_sign;
//extern unsigned int velocity, pre_pulse;
...
unsigned long millis(void) { return localTime; }

void SysTick_Handler(void)
{
	register struct SysTickTimers_s *vars = &SysTickTimers;
	localTime ++ ; // localTime plus plus affter 1 millisecond
	if (vars->Timer != 0)
		vars->Timer--;
}
...
/*******************************************************************************
* Function Name  	: HW_IRQHandler
* Return         	: None
* Parameters 		: None
* Created by		: Hoang Van Loi
...
{ 
  if (TIM_GetITStatus(HW_TIMER, TIM_IT_Update) == SET)
	{
    HandwheelUpdate();   
		TIM_ClearITPendingBit(HW_TIMER, TIM_IT_Update);
	}
}
...
#include "encoder_x.h"
void ENX_IRQHandler(void)
{
	if (TIM_GetITStatus(ENX_TIMER, TIM_IT_Update))
	{
		EncoderXUpdate();
		TIM_ClearITPendingBit(ENX_TIMER, TIM_IT_Update);
	}
}
...
* Notes			: 
*******************************************************************************/
extern ring_buffer_typdef Console_Rx_buffer;
void CONSOLE_IRQHanler(void)
{
	if(USART_GetITStatus(CONSOLE_USART, USART_IT_RXNE) != RESET)
  {
		char c = USART_ReceiveData(CONSOLE_USART);
		ring_buffer_push(&Console_Rx_buffer, c);
		//USART_ClearFlag(CONSOLE_USART, USART_FLAG_RXNE | USART_IT_RXNE);
	}
}
